...
class _MShipmentsTabState extends State<MShipmentsTab> with SingleTickerProviderStateMixin {
  ...
  Future<void> _load() async {
    ...
    if (mounted) { setState(() { _all = data; _loading = false; _applyFilter(); }); }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        elevation: 0,
        title: Text('My Shipments', style: GoogleFonts.inter(fontSize: 17, fontWeight: FontWeight.w700, color: Colors.white)),
        bottom: TabBar(
          controller: _tabCtrl,
        ...
        children: [
          ...
          Container(
            ...
            child: TextField(
              ...
              onChanged: (v) { _search = v.toLowerCase(); _applyFilter(); },
            ...
            child: _loading
                ...
                : _filtered.isEmpty
                    ? Center(child: Column(mainAxisSize: MainAxisSize.min, children: [
                        ...
                        Text('No shipments found', style: GoogleFonts.inter(fontSize: 16, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
                      ]))
                    : RefreshIndicator(
                        ...
                        child: ListView.separated(
                          ...
                          itemBuilder: (_, i) => _ShipmentListTile(shipment: _filtered[i]),
                        ),
                      ),
          ),
        ],
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          Text(shipment.displayCode, style: GoogleFonts.inter(fontSize: 13, fontWeight: FontWeight.w800, color: AppColors.primaryAmber, letterSpacing: 0.5)),
          const Spacer(),
        ...
        Row(children: [
          ...
          Text(shipment.pickupCity, style: GoogleFonts.inter(fontSize: 13, fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
          ...
          const SizedBox(width: 6),
          Text(shipment.receiverCity, style: GoogleFonts.inter(fontSize: 13, fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
        ]),
        const SizedBox(height: 6),
        Row(children: [
          const Icon(Icons.inventory_2_outlined, size: 12, color: AppColors.textMuted),
          const SizedBox(width: 4),
          Text(shipment.goodsDescription, style: GoogleFonts.inter(fontSize: 11, color: AppColors.textMuted)),
          const SizedBox(width: 12),
          const Icon(Icons.calendar_today_outlined, size: 11, color: AppColors.textMuted),
          const SizedBox(width: 4),
          Text(DateFormat('d MMM').format(shipment.createdAt), style: GoogleFonts.inter(fontSize: 11, color: AppColors.textMuted)),
        ]),
      ]),
    );
  }
