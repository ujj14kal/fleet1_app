...
class _THandoverTabState extends State<THandoverTab>
  ...
  Widget build(BuildContext context) {
    final received = _handovers
        .where((h) => h['to_transporter_id'] == _transporter?.id)
        .toList();
    final given = _handovers
        .where((h) => h['from_transporter_id'] == _transporter?.id)
        .toList();
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: _transporter == null
                ? null
                : () => _showCreateHandoverSheet(context),
      ...
      body: _loading
          ? const Center(child: TruckLoader(message: 'Loading handovers...'))
          : TabBarView(
              controller: _tabCtrl,
              children: [
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            Text(
              isReceived ? 'No handovers received' : 'No handovers given',
              style: GoogleFonts.inter(
                fontSize: 15,
                fontWeight: FontWeight.w700,
            ...
            children: [
              Row(
                children: [
                  ...
                  Text(
                    isReceived ? 'RECEIVED FROM' : 'HANDED TO',
                    style: GoogleFonts.inter(
                      fontSize: 11,
                      fontWeight: FontWeight.w700,
                      letterSpacing: 0.8,
              ...
              if (h['remarks'] != null &&
                  (h['remarks'] as String).isNotEmpty) ...[
                const SizedBox(height: 6),
                Text(
                  h['remarks'] as String,
                  style: GoogleFonts.inter(
...
class _CreateHandoverSheetState extends State<_CreateHandoverSheet> {
  ...
  Future<void> _load() async {
    final asgn = await ShipmentService.getTransporterAssignments(
      widget.transporter.id,
    );
    final trps = await ShipmentService.getAllTransporters();
    final activeShipments = asgn
        .map((a) => a['shipments'] as Map<String, dynamic>?)
        .where((s) => s != null && s['status'] != 'delivered')
        .map((s) => s!)
        .toList();
    if (mounted)
      setState(() {
        _activeShipments = activeShipments;
        _allTransporters = trps
            .where((t) => t['id'] != widget.transporter.id)
            .toList();
        _loading = false;
      });
  }
  ...
  Future<void> _submit() async {
    ...
    await ShipmentService.createHandover(
      ...
      handoverLocation: _locationCtrl.text.trim().isEmpty
          ? null
          : _locationCtrl.text.trim(),
      goodsCondition: _conditionCtrl.text.trim().isEmpty
          ? null
      ...
      child: Container(
        ...
        child: _loading
            ? const SizedBox(
                height: 120,
                child: Center(
                  child: TruckLoader(message: 'Loading handover options...'),
                ),
              )
            : Column(
                mainAxisSize: MainAxisSize.min,
                ...
                children: [
                  ...
                  Text(
                    'Transfer a shipment to another transporter',
                    style: GoogleFonts.inter(
                      fontSize: 12,
                      color: AppColors.textMuted,
                    ),
                  ...
                  DropdownButtonFormField<String>(
                    ...
                    onChanged: (v) => setState(() => _selectedShipmentId = v),
                  ...
                  DropdownButtonFormField<String>(
                    value: _selectedToTransporterId,
                    onChanged: (v) =>
                        setState(() => _selectedToTransporterId = v),
                    decoration: _dropDecor(),
                    hint: Text(
                      'Select transporter',
                      style: GoogleFonts.inter(
                    ...
                    items: _allTransporters
                        .map(
                          (t) => DropdownMenuItem(
                            value: t['id'] as String,
                            child: Text(t['company_name'] as String? ?? '—'),
                          ),
                        )
                        .toList(),
                  ),
  ...
  Widget _DropLabel(String t) => Padding(
  ...
  InputDecoration _textDecor(String hint, IconData icon) => InputDecoration(
    hintText: hint,
    filled: true,
    fillColor: AppColors.background,
    border: OutlineInputBorder(
